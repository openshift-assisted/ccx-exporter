...
func NewDurationMetricsDecoratorProcessing[Payload any](p Processing[Payload], registry *prometheus.Registry, config MetricsConfig) (Processing[Payload], error) {
...
type errorCountDecorator struct {
	processing Processing[ErrProcessingError]
	counter    *prometheus.CounterVec
}

func NewErrorCountDecoratorProcessing(p Processing[ErrProcessingError], registry *prometheus.Registry, config MetricsConfig) (Processing[ErrProcessingError], error) {
	ret := errorCountDecorator{
		processing: p,
	}
...
func (p errorCountDecorator) Process(ctx context.Context, processingError ErrProcessingError) error {
	p.counter.WithLabelValues(processingError.Category).Inc()

	return p.processing.Process(ctx, processingError)
}
