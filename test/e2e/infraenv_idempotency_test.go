...
var _ = Describe("Checking infraenv idempotency", func() {
	...
	BeforeEach(func() {
		...
		testConfig = e2e.CreateTestConfig("idem-infraenv")
	...
	Context("having already processed a valid infra_env event", func() {
		BeforeEach(func() {
			...
			Eventually(func(g Gomega, ctx context.Context) {
				for i, bucket := range testConfig.OutputS3Buckets {
					...
					g.Expect(objects[0]).To(ContainSubstring(e2e.S3Path(e2e.EventTypeInfraEnvs, e2e.EventDate, i)))
