...
var _ = Describe("Checking cluster state event with no host", func() {
	...
	BeforeEach(func() {
		...
		testConfig = e2e.CreateTestConfig("no-host-cluster")
	...
	When("pushing a cluster state event", func() {
		BeforeEach(func() {
			err := testContext.PushFile(ctx, "resources/input/cluster2.kafka.json")
			Expect(err).NotTo(HaveOccurred())
		})
